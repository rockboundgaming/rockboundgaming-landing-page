...
function displayFeaturedCreators(creators) {
  ...
  container.innerHTML = creators.map(c => {
    ...
    if (isDevelopment) {
      ...
      embedHTML = `
        <a href="https://twitch.tv/${c.twitch}" target="_blank" rel="noopener noreferrer" class="twitch-player-link" style="display: block; padding: 40px; background: linear-gradient(135deg, #9146ff 0%, #6d28d9 100%); border-radius: 8px; text-align: center; text-decoration: none; color: white; font-size: 18px; font-weight: bold;">
          <i class="fas fa-play-circle" style="font-size: 48px; margin-bottom: 20px; display: block;"></i>
          Watch ${c.name} Live on Twitch
        </a>
      `;
    }
